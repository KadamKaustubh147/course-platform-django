...
def get_cloudinary_image_object(instance, field_name="images", as_html=False, width=1200):
    # pass
    if not hasattr(instance, field_name):
        return ""
    image_object = getattr(instance, field_name)
    if not image_object: # this is for the displaying the course thumbnail image as field is images, abhi kyuki image_object use kar rahe toh common hai image object
        return ""
    image_options = {
        "width": width
    }
...
def get_cloudinary_video_object(
    ...
    if as_html:
        template_name = "videos/snippets/embed.html" # as template directory already settings.py mei hai
